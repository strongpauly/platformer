...
class Game extends React.Component<IGameProps, any> {
  ...
  private checkDoors() {
    ...
    if (door) {
      ...
      if (
        door.to &&
        this.props.player.x + this.props.player.width / 2 ===
  ...
  private moveLeft() {
    const player: IPlayer = this.props.player;
    this.step(
      player.x <= Constants.STEP_WIDTH ? 0 : player.x - Constants.STEP_WIDTH,
      true
    );
  }
      ...
      this.stepInterval = setInterval(() => {
        ...
        if (!collision.canMove) {
          ...
          if (newX + levelOffset <= bufferSize && levelOffset < 0) {
            ...
            newX >= viewWidth - bufferSize &&
            increment > 0 &&
            levelOffset + this.props.level.width > viewWidth
          ) {
            /* Scroll right */
            levelOffset -= increment;
          }
        ...
        if (
          collision.canMove &&
          !isNaN(collision.newY) &&
          collision.newY !== player.y &&
          !player.jumping &&
          stepping
        ) {
          this.fall();
